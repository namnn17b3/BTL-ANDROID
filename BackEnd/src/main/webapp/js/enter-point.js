...
function savePoint(pointId, modalInp, pointInfo) {
    ...
    fetch(`${window.location.protocol}//${window.location.host}/api/teacher/save-point`, requestOptions)
...
function renderPagination(data) {
    ...
    const allPage = quantity % itemInPage == 0
        ? parseInt(quantity / itemInPage) | 0
        : parseInt(quantity / itemInPage) + 1;
...
function renderTable(data) {
    ...
    studentPointTables.forEach(item => {
        ...
        Object.keys(student).forEach(key => {
			if (['email', 'phone', 'password', 'address', 'gender'].includes(key)) {
				return;
			}
            html += `
    ...
    btnEditPoints.forEach(item => {        
        item.addEventListener('click', () => {
            ...
            const currentSubjectKeys = ['percentCC', 'percentBTL', 'percentTH', 'percentKTGK', 'percentKTCK'];
            ...
            modalInp.forEach((item, index) => {
				if (currentSubject[currentSubjectKeys[index]] == 0) {
					item.value = '';
					item.disabled = true;
					item.readonly = true;
				}
...
function renderSelectionSubject() {
    ...
    fetch(`${window.location.protocol}//${window.location.host}/api/teacher/subject?teacherId=${teacherId}&termId=${termId}`, requestOptions)
        ...
        .then((result) => {
            ...
            for (let i = 0; i < data.length; i++) {
                html += `                
                    <option value="${data[i].id}" ${subjectId == data[i].id ? 'selected' : ''}>${data[i].name} (${data[i].numberOfCredits} TC)</option>
                `;
            }
